package com.example.facerecognitioncomparison.core
...
class LivenessDetector(vararg tasks: DetectionTask) {
    ...
    fun process(faces: List<Face>?, detectionSize: Int) {
        ...
        if (task.process(face)) {
            listener?.onTaskCompleted(task, taskIndex == tasks.lastIndex, face)
            taskIndex++
        }
    }
    ...
    interface Listener {
        ...
        fun onTaskCompleted(task: DetectionTask, isLastTask: Boolean, face: Face?)
